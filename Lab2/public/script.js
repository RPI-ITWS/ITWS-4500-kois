...
function displayNews() {
    ...
    pageArticles.forEach(article => {
        ...
        div.innerHTML = `<p>ID: ${article.source.id || "Unknown ID"}</p><h2>${article.title}</h2><p>${article.description}</p>`;
    ...
    fetch(`/api/articles/news?title=${encodeURIComponent(title)}`)
        ...
        .then(articles => {
            ...
            articles.forEach(article => {
                ...
                articleElement.innerHTML = `<a>ID:${article.source.id || "Unknown ID"}</a><p>${article.title}</p>`;
...
function deleteArticle(id) {
    ...
    console.log('Deleting article with ID:', idnum);
    
    fetch(`/api/articles/news.id=${encodeURIComponent(idnum)}`, {
        method: 'DELETE',
    })
    .then(response => {
        if (response.ok) {
